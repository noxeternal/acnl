class fishCtl {
  ...
  getFish () {
    this.http.get('res/fish.json').then((res) => {
      ...
      this.fishList.forEach(f => {
        if (this.shadows.indexOf(f.shadow) === -1) { this.shadows.push(f.shadow) }
        if (this.locations.indexOf(f.location) === -1) { this.locations.push(f.location) }
      })
    })
  }
  ...
  showFish () {
    if (this.fishList.length === 0) { return }

    let filteredFish = this.filterByName(this.fishList)
		// console.log('AfterName', filteredFish)

    if (typeof (filteredFish) === 'object' && filteredFish.length > 0) { filteredFish = this.filterByBells(filteredFish) }
		// console.log('AfterBells',filteredFish)

    if (typeof (filteredFish) === 'object' && filteredFish.length > 0) { filteredFish = this.filterByLocation(filteredFish) }
		// console.log('AfterLocation',filteredFish)

    if (typeof (filteredFish) === 'object' && filteredFish.length > 0) { filteredFish = this.filterByShadow(filteredFish) }
		// console.log('AfterShadow',filteredFish)

    if (typeof (filteredFish) === 'object' && filteredFish.length > 0) { filteredFish = this.filterByMonth(filteredFish) }
		// console.log('AfterMonth',filteredFish)

    if (typeof (filteredFish) === 'object' && filteredFish.length > 0) { filteredFish = this.filterByHour(filteredFish) }
  ...
  filterByName (fishList) {
    if (this.filterName === '') { return fishList }
  ...
  filterByBells (fishList) {
    if (this.filterBells.val === 0) { return fishList }
  ...
  filterByLocation (fishList) {
    if (this.filterLocation === '') { return fishList }
  ...
  filterByShadow (fishList) {
    if (this.filterShadow === '') { return fishList }
